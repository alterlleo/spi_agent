...
The **Serial Peripheral Interface (SPI)** is a synchronous, full-duplex communication protocol for fast, short-distance data exchange. It operates on a **Master-Slave** architecture, where a Master device (e.g., the Raspberry Pi) dictates the timing, and one or more Slave devices (e.g., microcontrollers) respond.
...
- **Speed**: User-Defined `SCK` signal frequency (i.e. 5MHz)
